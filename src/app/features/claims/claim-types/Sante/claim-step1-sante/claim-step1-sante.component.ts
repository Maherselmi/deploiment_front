...
// de déclaration d’un sinistre santé
@Component({
...
export class ClaimStep1SanteComponent implements OnInit {
  ...
  // Liste des types de sinistres santé proposés à l’utilisateur
  healthClaimTypes = [
  ...
  // pendant la déclaration d’un dossier santé
  ...
  // Objet contenant les informations du sinistre santé saisies dans le formulaire
  claim: ClaimData = {
    policyId: null,
    clientId: null,
    incidentDate: '',
  ...
  // Liste des polices santé disponibles et police actuellement sélectionnée
  policies: Policy[] = [];
  ...
  // Elle permet de charger les polices santé du client connecté
  ngOnInit(): void {
    this.loadPolicies();
  }
  ...
  // Méthode responsable du chargement des polices santé du client connecté
  // Elle récupère l’email depuis le localStorage puis filtre les contrats actifs de type SANTE
  loadPolicies(): void {
  ...
  // Méthode appelée lorsque l’utilisateur sélectionne une police santé
  ...
  // Elle vérifie la mutuelle, le client, la date des soins, le type de soins et la description
  validate(): boolean {
    this.errorMessage = '';
  ...
  // Elle valide le formulaire, crée le dossier santé, sauvegarde son identifiant puis redirige vers l’étape 2
  next(): void {
    this.errorMessage = '';
    this.successMessage = '';
