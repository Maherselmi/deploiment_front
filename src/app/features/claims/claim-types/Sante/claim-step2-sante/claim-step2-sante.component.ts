...
// Interface représentant un conseil affiché dans la zone d’envoi des documents médicaux
interface UploadTip {
  title: string;
  text: string;
  icon: string;
...
export class ClaimStep2SanteComponent implements OnInit {
  ...
  // dans l’ajout des documents médicaux
  ...
  // Getter qui indique si l’utilisateur peut envoyer les documents médicaux
  ...
  // Méthode permettant de revenir à la première étape de déclaration santé
  back(): void {
    this.router.navigate(['/Sante/step1']);
  }

  // Méthode principale d’envoi des documents médicaux
